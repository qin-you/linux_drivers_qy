...
/*****************************************************************************
 * 阻塞方式访问IO设备 （驱动中实现阻塞逻辑） 按键弹起来之前进程都是阻塞在读函数中
 * 从irq项目更改而来，已有按键驱动，定时器消抖逻辑。这里增加等待队列实现的阻塞逻辑即可
 * 阻塞/非阻塞访问IO本身和 中断 定时器不是必须联用，但和中断确实是常联用
******************************************************************************/
...
static ssize_t myirq_read(struct file *filp, char __user *buf, size_t cnt, loff_t *offt)
{
        ...
        if (atomic_read(&dev->releasekey) == 0) {
                ...
                // 这里是后来被调回来的 有可能是别的进程用信号杀本进程 有可能是被唤醒后正常调度的 需要判断
                if (signal_pending(current)) {
                        ret = -ERESTARTSYS;
                        goto wait_error;
                }
                // 执行到这里说明是wake_up后被正常调度的
