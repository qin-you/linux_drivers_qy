...
/*****************************************************************************
 * 异步通知实验 从noblockio实验修改而来 
 * 本质是信号机制，不需要应用层一直read尝试，驱动中实现最朴素的非阻塞IO就行（不用poll）
******************************************************************************/

#define IRQ_CNT         1
#define IMX6UIRQ_NAME   "fasyncnoti"
...
struct irq_dev {
        ...
        struct fasync_struct *fasync_queue;      // 异步通知相关结构体 作用？
};
...
static int myirq_fasync(int fd, struct file *filp, int on)
{
...
static int myirq_release(struct inode *inode, struct file *filp)
{
        return myirq_fasync(-1, filp, 0);                               // ?
}
