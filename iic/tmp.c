...
static unsigned char _ap3216c_read_reg(struct ap3216c_dev *dev, u8 reg)
{
        u8 buf = 0;
        _ap3216c_read_regs(dev, reg, &buf, 1);
        return buf;
}
...
static ssize_t ap3216c_read(struct file *filp, char __user *buf, size_t cnt, loff_t *loff)      // buf类型固定就是char 字节 cnt是字节数
{
        ...
        for (i = 0; i < 6; i++) {
                data[i] = _ap3216c_read_reg(dev, AP3216C_IRDATALOW);
        }
        ...
        if (data[4]&(1<<6))
                ...
                dev->ps = (((unsigned short)data[5] & 0b0011111) << 4) | (data[4] & 0x0f);
        ...
        /* 拷贝到用户空间 */
        ret = copy_to_user(buf, (char*)res, sizeof(res));
        printk("222\r\n");
        return 0;
}
...
{
        printk("locate: ap3216c_remove in xxxdriver.ko\r\n");
