...
static irqreturn_t key0_handler(int irq, void *pdev)    
{
        ...
        dev->curkeynum = 0;
        schedule_work(&dev->irqkeydesc[0].work);
        return IRQ_HANDLED;
}
...
static void work_func(struct work_struct *work)
{
        ...
        mod_timer(&mydev.timer, jiffies + msecs_to_jiffies(10));
}
...
static int keyio_init(void)
{
        ...
        /* 初始化KEY0的work */
        INIT_WORK(&mydev.irqkeydesc[0].work, work_func);
...
static int __init myirq_init(void)
{
        ...
        if (mydev.major) {
                ...
                register_chrdev_region(mydev.devid, IRQ_CNT, IRQ_NAME);
        } else {
                alloc_chrdev_region(&mydev.devid, 0, IRQ_CNT, IRQ_NAME);
        ...
        /* 创建设备节点 */
        mydev.class = class_create(THIS_MODULE, IRQ_NAME);
        if (IS_ERR(mydev.class))
                return PTR_ERR(mydev.class);
        mydev.device = device_create(mydev.class, NULL, mydev.devid, NULL, IRQ_NAME);
