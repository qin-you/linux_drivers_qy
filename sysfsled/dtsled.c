 ...
 * 手动增加sysfs中的led属性 mybrightness来控制灯亮灭
*********************************************************************/
...
struct dtsled_dev {
        ...
        struct device_attribute mybrightness;
};
...
ssize_t bright_show(struct device *dev, struct device_attribute *attr, char *buf)
{
        // dump_stack();
        u32 val = 0;
        val = readl(GPIO1_DR);
...
ssize_t bright_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
        // dump_stack();
        u32 brightness = 0;
        u32 ret = 0;
        // printk("brightness store\n");
        ...
        if (brightness == 1) {
                ...
                printk("brightness invalid\n");
                return -EINVAL;
        }
        return count;                   // return 0 means failed, this causes repeating echo x > brightness
}
...
static int __init led_init(void)
{
        ...
        // 5. 创建sysfs属性文件
        dtsled.mybrightness.show = bright_show;
        dtsled.mybrightness.store = bright_store;
        dtsled.mybrightness.attr.mode = 0666;
        dtsled.mybrightness.attr.name = "mybrightness";

        if (device_create_file(dtsled.device, &dtsled.mybrightness) < 0) {
                printk("device_create_file create attr file failed\n");
        }
...
{
        // 0. 删除属性文件
        device_remove_file(dtsled.device, &dtsled.mybrightness);
